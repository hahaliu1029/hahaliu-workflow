...
- `full(auto)` 因为“不向用户提问”而连实现前澄清和独立模型复核一起跳过。
...
8. auto 模式下保留独立复核下限，不能用主链自审或同模型子代理冒充第二意见。
...
`auto` 只取消中途向用户提问，不取消澄清、评审和完成证据。`full(auto)` 交付，以及触及认证、密钥、持久化格式或公共契约的高风险 `focused(auto)` 改动，至少需要一次由非主链模型完成的独立复核：

- 评审或第二意见必须显式选择不同模型；继承主链模型的子智能体不算独立复核。
- 需求已经明确、澄清链判定为 `skip` 时，独立复核下限仍然存在。
- 跨供应商咨询只有在当前请求或项目规则明确授权数据出域时才可使用；否则优先使用同供应商的不同模型。
- 没有可用的不同模型时可以保留授权范围内的部分成果，但必须报告复核缺口并以 `BLOCKED`/未完成状态交付，不能静默声称完整闭环。

用户明确要求新建独立项目或目录时，新目录内首次新增文件可不计入默认文件数和首次新增内容的 diff 预算；新目录外的修改预算仍为 0，已创建文件的反复重写仍计入 diff 预算。
...
| Codex CLI | Claude Code | 只读第二意见、架构挑战或救援通道 | 优先使用宿主可用的不同模型；仍不可用时报告复核缺口，不伪造跨模型意见 |
...
- auto 模式不会吞掉独立复核；不同模型不可用时不得把主链自审包装成完整复核。
