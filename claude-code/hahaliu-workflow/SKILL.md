...
- **复核不随澄清链跳过而消失**: auto 下的 full 交付,以及触及认证/密钥/持久化格式/公共契约的高风险 focused 改动,必须至少有一次非主链模型的独立评审或第二意见。成熟度门禁判「需求已明确→直接计划」只代表澄清链可跳过,不代表独立复核可归零;主链自审或继承主链模型的子代理均不算。评审模型必须显式指定,跨供应商咨询仍须先获数据出域授权。
