...
- 仅用户本次明确要求 auto/无人值守/中途不要提问时进入；同一消息有“先问我”“确认后再继续”等阻塞检查点时不进入。
- 所有原本向用户提出的澄清问题改为一次性提交给非主链模型；主链裁决后记录假设、依据、答者、置信度与翻案点。
- full 交付和高风险 focused 改动至少要有一次不同模型的独立复核。成熟度判“需求明确→跳过澄清”只取消访谈，不取消复核。
- 独立复核子智能体必须 fresh context，且在平台支持时显式指定不同模型；跨供应商只在数据出域获授权时使用。
- 缺少独立模型时允许继续整理部分成果，但完成状态必须是 BLOCKED/未完成，并写清未复核范围；不得用主链自审冒充。
...
- auto 下承担独立复核的子智能体必须使用非主链模型；澄清 skip 不会取消该门禁。
