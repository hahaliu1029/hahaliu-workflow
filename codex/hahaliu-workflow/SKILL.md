...
只有用户本次消息明确要求 auto、无人值守或中途不要提问时才进入；不能从“一次完成”等完成度措辞自行推断。auto 只取消中途向用户提问，不取消澄清、授权、评审与完成证据：

- 澄清问题改为由非主链模型一次性独立回答，主链采纳、驳回或降置信后写入假设台账；需要产品判断但无法取得独立回答时，不静默自决。
- full 交付，以及触及认证、密钥、持久化格式或公共契约的高风险 focused 改动，至少需要一次非主链模型的独立评审或第二意见。需求已明确、澄清链判 skip 时，该复核下限仍然存在。
- 独立复核优先使用 fresh-context 子智能体，并在宿主支持时显式指定不同于主链的模型；默认继承主模型、主链自审或只改变 reasoning effort 均不算不同模型。跨供应商咨询只有在当前请求或项目规则明确授权数据出域时才可使用。
- wayfinder 级巨大模糊任务只产出假设、分阶段计划和风险后停止。没有可用的不同模型时可以保留授权范围内的部分成果，但必须报告复核缺口并以 BLOCKED/未完成状态交付，不能声称 full 已闭环。
- 用户明确要求新建独立项目/目录时，新目录内首次新增文件可豁免默认文件/diff 数量预算；新目录外修改预算为 0，已创建文件的后续重写仍计入 diff。
